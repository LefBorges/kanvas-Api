...
class StudentCourseView(RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        emails = []
        students_course = serializer.validated_data.pop("students_courses")
        email = students_course[0]["student"]["email"]
        try:
            course = Course.objects.get(pk=self.kwargs["course_id"])
            students = Account.objects.get(email=email)
            StudentCourse.objects.create(course=course, student=students)
